...
import { MAX_REQUEST_BYTES, MAX_RESPONSE_BYTES, NATIVE_PROVIDER_INSTANCE_HEADER, NativeProviderProtocolError, parseNativeProviderCatalog, parseNativeProviderChunk, parseNativeProviderRequest, } from './native-provider-protocol.js';
...
function configuration(value) {
    if (!value ||
        typeof value !== 'object' ||
        Array.isArray(value) ||
        Object.keys(value).some(key => !['socketPath', 'requestTimeoutMs', 'maxConcurrentRequests'].includes(key)) ||
        ...
        throw new Error('Native provider socketPath must be an absolute canonical Unix socket path of at most 100 bytes.');
    }
...
async function prepareDirectory(socketPath) {
    ...
    for (const segment of parent
        ...
        if (existing && (!existing.isDirectory() || existing.isSymbolicLink())) {
            throw new Error('Native provider socket directory cannot contain symbolic links.');
        }
    }
    ...
    if (!info.isDirectory() ||
        ...
        throw new Error('Native provider socket directory must be physical, owned by the current user, and mode 0700.');
    }
    ...
    if (existing)
        throw new Error('Native provider socket path already exists; refusing to replace it.');
    return uid;
}
...
async function requestJson(request) {
    ...
    if (length && (!/^\d+$/.test(length) || Number(length) > MAX_REQUEST_BYTES)) {
        throw new NativeProviderProtocolError('Native provider request exceeds its byte limit.', 413);
    }
    if (request.headers['content-type']?.split(';')[0].trim() !==
        'application/json' ||
        request.headers['content-encoding']) {
        throw new NativeProviderProtocolError('Native provider requests require unencoded application/json.', 415);
    }
    const chunks = [];
    let size = 0;
    ...
    try {
        ...
        throw new NativeProviderProtocolError('Native provider request must be valid UTF-8 JSON.');
    }
}
    ...
    await ctx.effect(async () => {
        ...
        const rotate = () => {
            ...
            for (const controller of active)
                controller.abort(new NativeProviderProtocolError('Native provider configuration changed; refresh its catalog.', 409));
        };
        const listeners = [
        ...
        const catalog = async (signal, selection) => {
            const generation = instanceId;
            const models = [];
            for (const provider of ctx.llm.listProviders()) {
                if (selection && provider.id !== selection.providerId)
                    continue;
                try {
                    const entries = await abortable(ctx.llm.listModels(provider.id), signal);
                    for (const entry of entries) {
                        if (selection && entry.id !== selection.model)
                            continue;
                        const resolved = await abortable(ctx.llm.resolveModelInfo(provider.id, entry.id, signal), signal);
                        models.push({
                    ...
                    ctx.logger.warn('Native provider catalog group is unavailable', provider.id, error instanceof Error ? error.message : String(error));
                }
            }
            signal.throwIfAborted();
            if (generation !== instanceId)
                throw new NativeProviderProtocolError('Native provider configuration changed; refresh its catalog.', 409);
        ...
        const handle = async (request, response) => {
            ...
            const timeout = setTimeout(() => controller.abort(new NativeProviderProtocolError('Native provider request timed out.', 504)), config.requestTimeoutMs);
            timeout.unref();
            const generation = instanceId;
            let bytesSent = 0;
            let finished = false;
            ...
            const writeChunk = async (value) => {
                ...
                if (bytesSent > MAX_RESPONSE_BYTES)
                    throw new NativeProviderProtocolError('Native provider response exceeds its byte limit.', 413);
            ...
            try {
                if (request.method !== 'POST' ||
                    ...
                    throw new NativeProviderProtocolError('Unknown native provider operation.', 404);
                }
                ...
                if (request.url === '/catalog') {
                    if (!body ||
                        ...
                        throw new NativeProviderProtocolError('Native provider catalog request must be an empty object.');
                    }
                ...
                if (!available.models.some(model => model.providerId === parsed.provider &&
                    model.model === parsed.model)) {
                    throw new NativeProviderProtocolError('The selected native provider model is unavailable.', 422);
                }
                if (generation !== instanceId)
                    throw new NativeProviderProtocolError('Native provider configuration changed; refresh its catalog.', 409);
                ...
                if (!finished)
                    throw new NativeProviderProtocolError('Native provider ended without a terminal finish.', 502);
                response.end();
            }
            catch (error) {
                ...
                if (!response.headersSent) {
                    const status = error instanceof NativeProviderProtocolError ? error.status : 502;
                    response.writeHead(status, {
                        'content-type': 'application/json',
                        connection: 'close',
                    });
                    response.end(JSON.stringify({
                        error: error instanceof NativeProviderProtocolError
                            ? error.message
                    ...
                    const chunk = {
                        ...
                        reason: {
                            kind: 'error',
                            failure: {
                                code: error instanceof NativeProviderProtocolError &&
                                    error.status === 409
                                    ? 'NATIVE_PROVIDER_CHANGED'
                                    : 'NATIVE_PROVIDER_STREAM_FAILED',
                                message: error instanceof NativeProviderProtocolError
                                    ? error.message
