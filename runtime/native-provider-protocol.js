...
function text(value, label, maximum = 2048, empty = false) {
...
function optionalInteger(value, key, minimum = 0) {
    if (value[key] !== undefined)
        integer(value[key], key, minimum);
}
...
function json(value, limit) {
    ...
    if (Buffer.byteLength(JSON.stringify(value)) > limit) {
        throw new NativeProviderProtocolError('Native provider message exceeds its byte limit.', 413);
    }
}
...
function message(value) {
    ...
    switch (source.kind) {
        ...
        case 'tool':
            ...
            break;
        case 'model':
            keys(source, ['kind', 'provider', 'model'], ['replayState'], 'model source');
    ...
    if (data.role === 'system' && source.kind !== 'plugin')
        fail('system source');
    if (source.kind === 'tool') {
...
export function parseNativeProviderRequest(value) {
    json(value, MAX_REQUEST_BYTES);
    const data = record(value, 'request');
    keys(data, ['provider', 'model', 'messages'], [
        'instanceId',
        'system',
        'tools',
        'temperature',
...
export function parseNativeProviderCatalog(value) {
    ...
    for (const entry of data.models) {
        ...
        keys(model, ['providerId', 'model', 'name', 'providerName'], ['contextWindow', 'defaultMaxTokens', 'reasoning'], 'catalog model');
        ...
        if (model.reasoning !== undefined) {
            ...
            for (const entry of reasoning.efforts) {
                ...
                if (efforts.has(effort.id))
                    fail('duplicate reasoning effort');
...
export function parseNativeProviderChunk(value) {
    json(value, MAX_CHUNK_BYTES);
    ...
    switch (data.type) {
        ...
        case 'tool-call-delta':
            keys(data, ['type', 'index', 'id', 'argumentsDelta'], ['name'], 'tool delta');
            integer(data.index, 'block index');
            text(data.id, 'tool call ID');
        ...
        case 'finish': {
            ...
            if (reason.kind === 'error' || reason.kind === 'aborted') {
                ...
                keys(failure, ['message', 'code'], ['status', 'providerRetryAfterMs', 'requestId', 'offloadImages'], 'failure');
